...
export const MovieDetails = ({title, image, rating, genre, duration, language, description, directors, caption}: Movie) => {
    return(
        <div className="flex flex-col lg:flex-row justify-start items-start gap-2 w-full">
            <img className="w-64 hidden lg:block aspect-[3/4] rounded-2xl" src={image.portrait} alt={`${title} poster`}/>
            <div className="flex flex-col items-center gap-4 flex-grow">
                <div className="flex items-start gap-2 self-stretch">
                    <span className="text-3xl leading-0 font-semibold text-white flex-grow">{title}</span>
                    <Rating rating={rating ? rating : 0}/>
                </div>
            <img className="w-full lg:hidden aspect-[4/3] rounded-2xl" src={image.landscape} alt={`${title} poster`}/>
